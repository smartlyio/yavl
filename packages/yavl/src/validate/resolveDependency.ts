...
const resolveDependency = <Data, ExternalData, ErrorType>(
  ...
  // Mutable traversal state — avoids allocating a new context object on each path step
  ...
  for (const pathPart of pathToDependency) {
    if (pathPart.type === 'field') {
      ...
      if (pathPart.focus === 'index') {
        ...
        // array.all focus — if there's no data at this path, reset paths since we're no longer focusing any data
        if (data === undefined) {
          data = [];
          paths = [];
          isFocusedOnSinglePath = false;
          ...
          if (isFocusedOnSinglePath) {
            // Each element of the array becomes its own tracked path, branching from the current single path
            const basePath = paths[0];
            paths = data.map((_: unknown, idx: number) => [...basePath, idx]);
          } else {
            // Already in multi-path mode — flatten nested arrays and expand paths per nested element
      ...
      const getIndexOfPath = (indexPath: DataPath) => {
