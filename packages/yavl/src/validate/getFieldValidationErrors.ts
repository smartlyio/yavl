...
export const getFieldValidationErrors = <ErrorType>(
  ...
  fieldCache.validations.forEach(({ parentDefinitions, isDependency }) => {
    ...
    processDependentPermutations(parentDefinitions, processingContext.data, indices, indexPermutation => {
      ...
      errorCache.forEach(({ field: fieldFromCache, errors: errorsFromCache }) => {
