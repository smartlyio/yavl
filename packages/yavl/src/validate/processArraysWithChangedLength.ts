...
export const processArraysWithChangedLength = <Data, ExternalData, ErrorType>(
  ...
  arraysWithChangedLength.forEach(arrayPath => {
    ...
    const dependentPathsWithArrayAll = dependentPaths.map(path => `${path}[all]`);

    // find all fields that depend on this array having items removed
    const fieldDependencies = Object.entries(processingContext.fieldDependencyCache).flatMap(
      ([field, dependencies]) => {
        if (!dependencies) {
          return [];
        }
        const fieldMatches = dependentPathsWithArrayAll.some(dependentPath => field.startsWith(dependentPath));
        if (!fieldMatches) {
          return [];
        }
        return dependencies;
