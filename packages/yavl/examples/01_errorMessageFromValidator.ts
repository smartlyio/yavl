...
const myModel = model<MyForm>((form, { field, validate, dependency }) => [
  field(form, 'name', name => [validate(name, name => name.length >= 5, 'Name must be at least 5 characters')]),

  field(form, 'password', password => [
    validate(password, isValidPassowrd),
    validate(password, dependency(form, 'passwordAgain'), (pw1, pw2) => pw1 !== pw2, 'Passwords do not match'),
  ]),
]);
