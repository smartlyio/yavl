...
const myModel = model<MyForm>((form, { field, withFields, array, validate }) => [
  field(form, 'users', users => [
    array(users, user => [
      field(user, 'name', name => [validate(name, name => name.length >= 5, 'Name must be at least 5 characters')]),

      withFields(user, ['password', 'passwordAgain'], ({ password, passwordAgain }) => [
        validate(password, isValidPassword, 'Invalid password'),
        validate(password, passwordAgain, areStringsEqual, 'Passwords do not match'),
      ]),
    ]),
  ]),
]);
