...
const testModel = model<TestModel>((root, model) => [
  model.withFields(
    root,
    ['field1', 'field2', 'fieldWithOnlyConditionalAnnotation', 'arrayWithCondition'],
    ({ field1, field2, fieldWithOnlyConditionalAnnotation, arrayWithCondition }) => [
      model.withFields(
        field1,
...
describe('annotation subscribers', () => {
  ...
  const subscribeToAnnotation = <T>(
    ...
    const unsubscribe = subscribeToFieldAnnotation(validationContext, field, annotation, subscriber, defaultValue as T);
  ...
  describe('subscribing to field annotations', () => {
    ...
    it('should instantly notify the current value', () => {
      ...
      expect(subscribers.field1AnnotationA.currentValue).toEqual('outside condition');

      expect(subscribers.field1AnnotationB.subscriber).toHaveBeenCalledTimes(1);
      expect(subscribers.field1AnnotationB.currentValue).toEqual('false branch');

      expect(subscribers.field2AnnotationA.subscriber).toHaveBeenCalledTimes(1);
      expect(subscribers.field2AnnotationA.currentValue).toEqual('false branch');
    });
    ...
    describe('when multiple annotations for same field change at the same time', () => {
      ...
      it('should notify both subscriber with updated values', () => {
        expect(subscribers.field1AnnotationA.currentValue).toBe('inside last condition');
        expect(subscribers.field1AnnotationB.currentValue).toBe('true branch');
      });
    ...
    describe('when annotation is only set for field when a condition is true', () => {
      ...
      describe('and when condition is initially false', () => {
        it('should notify subscriber with the default value if one is defined', () => {
          const subscriber = subscribeToAnnotation('fieldWithOnlyConditionalAnnotation', testAnnotationA, 'default');
        ...
        it('should throw an error if no default value is defined', () => {
          expect(() =>
            subscribeToAnnotation('fieldWithOnlyConditionalAnnotation', testAnnotationA),
          ).toThrowErrorMatchingInlineSnapshot(`"Annotation "A" not found for fieldWithOnlyConditionalAnnotation"`);
        });
      });
      ...
      describe('when condition is initially true and changes to false', () => {
        ...
        it('should throw an error if no default value is defined', () => {
          // add subscriber
          const subscriber = subscribeToAnnotation('fieldWithOnlyConditionalAnnotation', testAnnotationA);
        ...
        it('should work for arrays and nested conditions', () => {
          // add subscriber
          const subscriber = subscribeToAnnotation('arrayWithCondition.array[0].value', testAnnotationA, 'default');
    ...
    describe('when subscribing to value inside array and array item is removed', () => {
      ...
      it('should not throw an error even if default value is missing', () => {
        // add subscriber
        const subscriber = subscribeToAnnotation('arrayWithCondition.array[1].value', testAnnotationA);
    ...
    describe('when unsubscribing', () => {
      ...
      it('should still notify other subscribers', () => {
        expect(subscribers.field1AnnotationA.subscriber).toHaveBeenCalledTimes(1);
        expect(subscribers.field1AnnotationA.currentValue).toBe('inside last condition');
