...
#
# Finds the last "chore: version packages" commit, queries for PRs merged
# after it, and picks the highest bump (major > minor > patch). PR titles
# are saved to /tmp/pr-entries.txt for inclusion in the version commit.
#
# Outputs:
#   bump  — written to $GITHUB_OUTPUT (patch, minor, major, or empty)
#
# Expects:
