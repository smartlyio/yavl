...
import {
  getResolvedValidationErrors,
  updateResolvedValidationErrors
} from '../utils/resolvedValidationsHelpers';
...
export const updateChangedValidation = <Data, ExternalData, ErrorType>(
  ...
  const previousErrors = getResolvedValidationErrors(
    processingContext.resolvedValidations.current,
    pathToFieldStr
  );
  ...
  if (!R.equals(nextErrors, previousErrors)) {
    updateResolvedValidationErrors(
      processingContext.resolvedValidations,
      pathToFieldStr,
      nextErrors
    );
  }
